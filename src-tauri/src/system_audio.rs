...
/// - macOS: Requires ScreenCaptureKit (macOS 12.3+) or virtual audio device
...
impl SystemAudioCapture {
    ...
    pub fn new(config: SystemAudioCaptureConfig) -> Result<Self, String> {
        ...
        // Check platform availability and get actual format
    ...
    fn check_availability(config: &SystemAudioCaptureConfig) -> Result<(bool, u32, u16), String> {
        ...
        {
            // Windows: Check for WASAPI loopback device
        ...
        #[cfg(target_os = "macos")]
        {
            // macOS: System audio capture requires ScreenCaptureKit (macOS 12.3+)
...
fn run_system_audio_capture(
    ...
    // Try to find a suitable device for system audio capture
    #[cfg(target_os = "linux")]
    let device = {
        host.input_devices()
            .map_err(|e| format!("Failed to enumerate devices: {}", e))?
            .find(|d| {
                d.name().map(|n| n.contains("monitor")).unwrap_or(false)
            })
    ...
    #[cfg(target_os = "macos")]
    let device = {
        // Try to find a virtual audio device first
        host.input_devices()
            .map_err(|e| format!("Failed to enumerate devices: {}", e))?
            .find(|d| {
                d.name().map(|n| {
    ...
    #[cfg(target_os = "windows")]
    let device = {
        // On Windows, use default output device for loopback
